...
class NMT(nn.Module):
    ...
    def step(
        ...
        ###     1. yield alpha_t
        alpha_t = torch.softmax(e_t, dim=1)  # alpha_t : (b, src_len)

        ###     2. obtain the attention output vector, a_t.
        alpha_t = torch.unsqueeze(alpha_t, 1)  # alpha_t       : (b, 1, src_len)
        a_t = torch.bmm(alpha_t, enc_hiddens)  # enc_hiddens   : (b, src_len, 2*h)
        # a_t           : (b, 1, 2*h)
        a_t = torch.squeeze(a_t, 1)  # a_t           : (b, 2*h)

        ###     3. compute tensor U_t
        dec_hidden = torch.squeeze(dec_hidden, 2)  # dec_hidden        : (b, h, 1)
        U_t = torch.cat((a_t, dec_hidden), 1)  # U_t           : (b, 3*h)
        ###     4. to compute tensor V_t
        V_t = self.combined_output_projection(
            U_t
        )  # combined_output_projection : (3h, h)
        # V_t                        : (b, h)
        ###     5. Compute tensor O_t
        O_t = torch.tanh(V_t)  # O_t                        : (b, h)
        O_t = self.dropout(O_t)  # O_t                        : (b, h)
        ### END YOUR CODE
